...
def Stage0_Json(input_str, index_line, keyword1, keyword2, RollingScoreBank):
	...
	if flag_TidTimeAuthor and flag_tagExist and flag_tag_keyword1 and flag_tag_keyword2:
		...
		for item in tag_list:
			if item in RollingScoreBank['tag_relevant1'] and RollingScoreBank['tag_relevant1'][item] >= 5.0:
				flag_tag_relevant1 = True
				break
		# check if there is high-tier tag of keyword2
		for item in tag_list:
			if item in RollingScoreBank['tag_relevant2'] and RollingScoreBank['tag_relevant2'][item] >= 5.0:
	...
	if flag_TidTimeAuthor and flag_tagExist and len(Tweet_OBJ['Tag_Keyword']) == 0 and len(Tweet_OBJ['Tag_Relevant']) == 0:
		if user_id in RollingScoreBank['user1'] and RollingScoreBank['user1'][user_id] >= 1.0:
			flag_due_user = True
		if user_id in RollingScoreBank['user2'] and RollingScoreBank['user2'][user_id] >= 1.0:
			flag_due_user = True
